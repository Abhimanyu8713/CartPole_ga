...
def select_pop(pop_new,sizePopulation,retain):
	...
	for i in range(sizePopulation-retain):
		pop1 = []
		#pop.append(np.random.rand(2) * 10 - 1)
		pop.append(np.random.rand(4) * 10 - 1)
...
def mutate_child(individual):
	if (np.random.rand() < 0.5):
		individual[0] = individual[0] + 10*0
		#print(individual[0])
	else:
		individual[len(individual)-1] = individual[len(individual)-1] + 10*0
